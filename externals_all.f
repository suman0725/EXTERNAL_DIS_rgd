...
c     CALL  FYXSEC8(E0SET,EPSET,TH,SIGMA_CORR)                     
     ...
     >        E0SET,EPSET,TH,SIGMA_BORN,sigma_corr
         ...
         dorad=.true.
         doext=.true.   ! RGD: internal+external radiation (upstream material not in GEMC)
...
cc also, all deltae_cc's are computed for Z-1, not Z!/-* 
              ...
              elseif(iA.eq.208) then !Lead	  
                 deltae_cc = 0.02035
              elseif(iA.eq.118) then !Tin (natural, avgA=118.71)
                 deltae_cc = 0.01440
              endif
...
cc also, all deltae_cc's are computed for Z-1, not Z!/-* 
	...
	elseif(iA.eq.197) then !Gold
      ...
      elseif(iA.eq.118) then !Tin (natural Sn, avgA=118.71)
           deltae_cc = 0.01440
	endif
